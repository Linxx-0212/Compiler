...
public class BuildSymTable extends DepthFirstVisitor {
    ...
    public void inVarDecl(VarDecl varDecl) {
        ...
        if (! this.isDuplicate(varDecl.getName())) {
            int offset = currentScope.getOffset();
            if (currentScope.isclass() == 1) {
                varSTE = new VarSTE(varDecl.getName(), getType(varDecl.getType()), "Z", offset-3,0);
            }
            else {
                varSTE = new VarSTE(varDecl.getName(), getType(varDecl.getType()),"Y",offset,0);
            }
            offset += this.getType(varDecl.getType()).getAVRTypeSize();
    ...
    public void inMethodDecl(MethodDecl methodDecl) {
        if(this.flag) {
            ...
            int offset = 3;
            methodSTE.setVarSTE(new VarSTE("This",Type.getClassType(this.currentClass),"Y",1,1));
            for (Formal formal1 : methodDecl.getFormals()) {
                ...
                if(methodSTE.getScope().lookup(formal1.getName())==null) {
                    methodSTE.setVarSTE(new VarSTE(formal1.getName(), this.getType(formal1.getType()), "Y", offset,1));
                    offset += this.getType(formal1.getType()).getAVRTypeSize();
                }
                else
                    throw new SemanticException(
                            "Duplicate Variable",
            ...
            methodSTE.getScope().setOffset(offset);
    ...
    public void inTopClassDecl(TopClassDecl topClassDecl) {
            ...
            if (!this.isDuplicate(topClassDecl.getName())) {
                ...
                this.symtable.getCurrentScope().setEnclosing(scope);
                this.symtable.getCurrentScope().setclass(1);
            }
            else {
                throw new SemanticException(

...
public class CheckTypes extends DepthFirstVisitor
{
   ...
   public void outEqualExp(EqualExp node) {
   	Type lexpType = this.mCurrentST.getExpType(node.getLExp());
   	//System.out.println(lexpType);
   	Type rexpType = this.mCurrentST.getExpType(node.getRExp());
   	//System.out.println(rexpType);
   	if (lexpType == rexpType||((lexpType == Type.BYTE && rexpType == Type.INT)||(rexpType == Type.BYTE && lexpType == Type.INT))||(lexpType == Type.INTARRAY &&(rexpType == Type.BYTE||rexpType == Type.INT))||(lexpType == Type.COLORARRAY && rexpType ==Type.COLOR)) {
   		this.mCurrentST.setExpType(node,Type.BOOL);
   	} else {
   		throw new SemanticException(
   ...
   public void outCallExp(CallExp node) {
	   ...
	   if(methodSTE != null) {
           ...
           while (args.hasNext() && param.hasNext()) {
               ...
               if (this.mCurrentST.getExpType(rexp) == lexp) {
                   if ((this.mCurrentST.getExpType(rexp) == Type.BYTE && lexp == Type.INT) || (this.mCurrentST.getExpType(rexp) == Type.INT && lexp == Type.BYTE)) {
                       throw new SemanticException(
                               "Wrong Args Type",
                               node.getLine(),
                               node.getPos()
   ...
   public void outNewArrayExp(NewArrayExp newArrayExp) {
	...
	if(getIType(newArrayExp.getType()) == Type.COLOR)
   ...
   public void outAssignStatement(AssignStatement node) {
	...
	public void outArrayExp(ArrayExp arrayExp)
	{
		...
		if(type == Type.INTARRAY)
		    mCurrentST.setExpType(arrayExp,Type.INT);
		else
		    mCurrentST.setExpType(arrayExp,Type.COLOR);
	}
